...
## 1.4.1
### Build & Compatibility
- **Android: the plugin now configures under Android Gradle Plugin 9.x** - Two independent hard errors stopped an app on AGP 9 from configuring at all. AGP 9 brings its own Kotlin support, so `apply plugin: 'kotlin-android'` is rejected outright (`The 'org.jetbrains.kotlin.android' plugin is no longer required for Kotlin support since AGP 9.0`); and AGP 9's DSL dropped the `compileSdkVersion` setter, so `compileSdkVersion 35` left the project with no `compileSdk` at all. The Kotlin plugin is now applied only when AGP's built-in Kotlin is not in play (AGP 8.x, or AGP 9.x with `android.builtInKotlin=false`), `compileSdk` uses the property form, and `kotlinOptions` moved to the top-level `kotlin.compilerOptions` block that both KGP and built-in Kotlin understand. Verified against AGP 9.4.0/Gradle 9.6.1 and AGP 8.6.0/Gradle 8.13 from the same file - **no change is required in host apps**.
- **Android: dead Android Gradle Plugin pin removed** - The plugin declared `classpath 'com.android.tools.build:gradle:8.7.0'` in its own `buildscript`. The host app's AGP always wins, so the pin protected nothing and only misled anyone reading the build file. The Kotlin Gradle plugin classpath is deliberately kept: it is what lets the plugin build inside host apps that do not declare KGP themselves.
- **Android: `allprojects { repositories { … } }` replaced with a plain `repositories` block** - Identical behaviour (the block only ever applied to the plugin's own project), minus the conflict with Gradle's Isolated Projects. JitPack still resolves in host apps that do not declare it.
- **Android: minimum supported AGP documented as 8.2.2** - Required by the bundled Kotlin Gradle plugin. This floor already existed; it was simply never written down. See the Requirements table in the README, which now also covers Gradle, JDK, `compileSdk`, `minSdk` and `targetSdk`.
- **Example app: `compileSdk` and `targetSdk` pinned to 36** - Previously inherited from `flutter.compileSdkVersion` / `flutter.targetSdkVersion`, which still resolve to 35. Google Play has required API 36 for new apps and updates since 31 Aug 2026, so the example was demonstrating a configuration Play no longer accepts.
- **Example app: Gradle 9 preparation** - `rootProject.buildDir` (removed in Gradle 9) replaced with `layout.buildDirectory`, and the deprecated Groovy space-assignment syntax replaced with explicit assignments across the example and plugin build files. The example's Gradle wrapper stays on 8.13 for now: Flutter 3.32's own Gradle plugin does not compile under Gradle 9.

### Dependencies
- **Android: native android-sdk 3.2.0 → 3.2.1** - Picks up a longer timeout and added diagnostics for notification image processing. The public API only gains two nullable `LiveActivity` fields, so nothing changes on the Dart side.
- **Android: Firebase consolidated onto BOM 34.7.0** - The plugin declared both `firebase-bom:33.16.0` and an explicit `firebase-messaging-ktx:24.1.2`; the explicit version silently won, making the BOM inert, and the `-ktx` artifacts have been merged into their base artifacts since BOM 33.x. Now a single BOM drives `firebase-messaging` (25.0.1). Verified not to raise the `compileSdk` or `minSdk` an app needs.
- **Android: Guava 31.1 → 33.7.1-android** - Still required for the `ListenableFuture` the native 3.x API returns.
- **`flutter_lints` 2.0.0 → 6.0.0** (dev dependency only; `flutter analyze` stays clean).
